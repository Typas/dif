...
lists tens of methods, so a full `u8` codec space (2⁸) is wanted; LZ4-fast
acceleration ranges up to ~2⁷, so a full `u8` level is wanted. One byte each.
...
- Add `--dif-codecs` to the `formats` subparser (default = all 7) so a run can
...
- **wasm decoder**: baseline gains Store/Deflate/Xz/**Lz4**. Lzav (and
  Zstd/Brotli) in-browser depend on the §2b Zig cross-compile landing; if it
